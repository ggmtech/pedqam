...
pacman::p_load( tidyverse, DiagrammeR,     networkD3 )      # For alluvial/Sankey diagrams

#library(grviz)

library(devEMF)  # install.packages("devEMF") #imported natively as vector graphics in both Libre/MS Office
...
digraph nicegraph {
  ...
  subgraph cluster0 { node [fixedsize = true, width = 3]
    '@@1-1' -> '@@1-2' -> '@@1-3' -> '@@1-4'
    '@@1-4' -> '@@1-5' -> '@@1-6' -> '@@1-7'
  }
...
sequenceDiagram
  ...
  ticket seller->>database: check seats
...
"digraph gkname2 {
    ...
    node [shape = rectangle, style = filled, fillcolor = LightBlue] # define the global default node styles #fillcolor = Linen
