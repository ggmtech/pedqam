...
d = {'cat': 'cute', 'dog': 'furry', 'puppy':'Bruce'}  # Create a new dictionary with some data
...
print(d['puppy'])       # Get an entry from a dictionary; prints "cute"
...
# use the dot function to compute inner products of vectors, to multiply a vector by a matrix, 
...
conda_create(scipy)
# scipy
from scipy.misc import imread, imsave, imresize   # unable to load????
img = imread('assets/cat.jpg') # Read an JPEG image into a numpy array
...
x = np.arange(0, 3 * np.pi, 0.1)   # Compute the x and y coordinates for points on a sine curve
y = np.sin(x)
