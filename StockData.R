...
packages <- c("tidyverse",   "anytime", "magrittr",  "here", "lubridate",
              ...
              # "graphlayouts",  "here", "interplot", "margins", 
...
here::here()  #getwd()
#setwd("/Users/gk/Google Drive") ; ls()  # files in memory ; list.files()# files of getwd()
##################################################################################
...
#gs4_auth(  email = gargle::gargle_oauth_email(learnit2040@gmail.com),  path = NULL,
#  subject = NULL,  scopes = "spreadsheets",  cache = gargle::gargle_oauth_cache(),
#  use_oob = gargle::gargle_oob_default(),  token = NULL)

# AllStockData google sheet in learnit
NSElistlink = ""
NSEbhavlink = ""

# My Stock Data Date 29 June
ss = "https://docs.google.com/spreadsheets/d/1xWD4Xn-qD_wsiLpl8TIDVta6uh5R7ZLYJgGzfG8Lijw/edit?gid=154910109#gid=154910109"
sheet_names(ss)  # see sheets names

RowStockData <-  googlesheets4::read_sheet(ss, sheet = "AllStockData") 
RowStockData
NSElist      <-  googlesheets4::read_sheet(ss, sheet = "NSElist",    col_names = TRUE,  skip = 0) 
NSElist

# , col_names = TRUE,  col_types = "c"  , skip = 1, trim_ws = TRUE, na = "")   # col_types = "cDDcccildd"
# library(snakecase)
...
names(RowStockData) %>% snakecase::to_snake_case() %>% dput()    ## c("some_b_ad_input", "some_good_input")


######################
# Data Analysis of RowStockData
#####################
RowStockData
RowStockData %>% names()
RowStockData %>% tibble::glimpse() #View()
RowStockData %>% View() #glimpse()
RowStockData %>% glimpse()
RowStockData %>% str()
RowStockData %>% filter( is.na("") )
RowStockData %>% summary 


RowStockData %>% xray::distributions()
RowStockData[ , c(16) ] %>% GGally::ggpairs()    # Draw pairs plot

RowStockData %>% GGally::ggpairs() 
RowStockData %>% GGally::ggpairs( columns = c(5, 6, 7)) 
RowStockData %>% GGally::ggpairs( columns = c("ROE", "OPM", "PEG"), 
                                  columnLabels = c("ROE", "OPM", "PEG"),
                                  mapping = aes(color = "pe")) 




RowStockData %>% plot_missing(  ggtheme = tidyquant::theme_tq(),
                                title = str_glue( 'Exploring Missing Data (N = { count(tmp) } )' )    
                              )

RowStockData %>% ggplot( aes(x = Name,  fill = Name)  ) +
...
#FullStockList <- full_join(RowStockData, NSElist , by = NULL, suffix = c(".x", ".y") )
FullStockList %>% View()
...
tmp %>% View()
NSElist %>% View()
StockListNSE <- left_join(NSElist ,RowStockData,  by = c("SYMBOL" = "NSE Code"), suffix = c(".x", ".y") )
StockListNSE %>% View()
# EMPadfailure <- rbind(EMPadfailuresWR, EMPadfailuresNR )
...
#tmp %>% separate_rows( "Date of manufacturing", sep = ",", convert = FALSE) -> tmp# %>% View() # 20361
...
#################
names(RowStockData)
tmp <- RowStockData %>% dplyr::select( "NSE Code", "MarketCap", "pe", "PEG", "OpCF3Y" , #FreeFloat
                                       "RoCE", "ROA", "ROE", "EPS", "PB", "Promoter", "EqShares",
                                       "Growth3Y", "Growth10Yr", "PGrowth3Y", "PGrowth10Yr", "BookValue",
                                       "DMA 200", "Price" ,
                                       "Return over 3years", "Return over 5years",
                                       "ATR", "CashConvCycle" , 
                                       "OPM last year" #important
                                       )  
...
# FailDate = lubridate::parse_date_time( paste( `Date of replacement`), orders = c("dmy", "mdy" ,"ymd"), tz = "UTC")) %>% View()
...
#EMpadlife <- tmp %>%  mutate(MakeDate = parse_date_time( paste( '01-',  str_trim(`Date of manufacturing`) , side = both) ), orders = c("dmy", "mdy" ,"ymd"  ) ),
#FailDate = parse_date_time( `Date of replacement` , orders = c("dmy", "mdy" ,"ymd"  ) ),
#lifedays =  #as.numeric( difftime(Datefail, MakeDate, days ) ),  as.numeric( (FailDate - MakeDate)/86400  ) ,
#Reason   = 'Type of failure',
#dcencer = if_else( lifedays > 1460, 0, 1  )                   )  %>%         
#  filter( !is.na(MakeDate )  )      %>%
#  filter( FailDate > as_date("31/03/2021")  )   %>%
#  filter( MakeDate < as_date("31/03/2016")  )   %>%
#  dplyr::select( Rly, MakeDate,  FailDate, Reason, lifedays, dcencer ) 
# EMpadlife
#################


# tmp %>% # separate("Date of manufacturing", into = c("made1", "made2", "made3"), sep = "," ) %>%             # View()
...
summarytools::view(dfSummary(tmp))
DataExplorer::create_report(tmp)


tmp %>% DataExplorer::introduce() # To get introduced to dataset:
tmp %>% DataExplorer::plot_intro() # to visulise above data 
tmp %>% DataExplorer::plot_str() 
tmp %>% DataExplorer::plot_missing( )
tmp %>% DataExplorer::plot_missing( ggtheme = tidyquant::theme_tq(),
                                    title = str_glue( 'Exploring Missing Data (N = { count(tmp) }   )' )    
                                  )
tmp %>% DataExplorer::plot_bar(). #To visualize frequency distributions for all discrete features:
tmp %>% DataExplorer::plot_boxplot( by = fy)
tmp %>% DataExplorer::plot_density()
tmp %>% DataExplorer::plot_histogram() # To visualize distributions for all continuous features:
tmp[6] %>% DataExplorer::plot_histogram() # To visualize distributions for all continuous features:

tmp %>% DataExplorer::profile_missing()
tmp %>% DataExplorer::group_category()
tmp %>% DataExplorer::plot_scatterplot()
tmp[6] %>% DataExplorer::plot_scatterplot()

tmp %>% DataExplorer::update_columns()


tmp %>% inspectdf::inspect_cat()
tmp %>% inspectdf::inspect_imb(tmp)
tmp %>% inspectdf::inspect_na(tmp)
tmp %>% inspectdf::inspect_types(tmp)

tmp %>% DataExplorer::plot_qq(by = "NSE Code", sampled_rows = 1000L)
tmp %>% DataExplorer::plot_qq(sampled_rows = 1000L)
DataExplorer::plot_correlation( na.omit(tmp), maxcat = 5L)

tmp2 <- tmp[, c("MarketCap", "pe", "RoCE", "ROA", "ROE", "EPS", "PB")]

plot_scatterplot(tmp2, by = "ROE", sampled_rows = 1000L)

# PCA analysi  plot_prcomp(na.omit(final_data)) directly, but PCA works better with cleaner data. 
# perform and visualize PCA on some selected features:
# pca_df <- na.omit(final_data[, c("origin", "dep_delay", "arr_delay", "air_time", "year_planes", "seats")])

plot_prcomp(tmp[], variance_cap = 0.9, nrow = 2L, ncol = 2L)



plot_scatterplot(tmp, by = "pe", sampled_rows = 1000L)


#create histogram of values for price
ggplot(data=tmp, aes(x = MarketCap, y=pe, color=MarketCap) ) +
  geom_point() +
...
ggplot(data=RowStockData, aes(x=Industry, y=pe)) + 
  geom_boxplot(fill="steelblue") +
  coord_flip() #  coord_flip()

RowStockData %>% names()
library(tidyverse)
RowStockData %>% dplyr::select('Name', 
             Name ,   Industry    ,    Price    ,
            MarketCap ,   pe ,  PEG   ,
            DER  ,  OpCF3Y     ,   Sales    ,  RoCE ,
            ROA   ,  ROE,  EPS  , PB       ,
            Promoter  ,    Pledge    ,  IntCoverR, PriceToCF   ,
            Growth3Y  ,    Growth5Y   ,   PGrowth3Y   , PGrowth5Y 
      
      ) %>%
  DataExplorer::plot_correlation(  maxcat = 5L)

####################################################

## PCA 
#calculate principal components
results <- prcomp(tmp, scale = TRUE)
results <- prcomp(RowStockData, scale = TRUE)
...
EMpadlife   %>% mutate( Make = fct_collapse(Make, 
  ...
  filter( as.numeric( year(Datefail) ) > 2015, 
          as.numeric( year(Datefail) ) < 2021) -> EMpadlife3 # %>% View()

EMpadlife3 %>% count( Make) %>% View()  
EMpadlife3 %>% str()  

###### Write to Sheet
# sheet_write(data, ss = NULL, sheet = NULL)
# already def sslife <- "https://docs.google.com/spreadsheets/d/1elSHjPakhrMHJsyGuP74FPm0NirIYPnT-DyQome48Lo/edit#gid=1349265385"

EMpadlife3 %>% googlesheets4::sheet_write( ss = sslife, sheet = "allmajor")

EMpadlife3 %>% count(FCY)
